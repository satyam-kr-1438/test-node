...
const Sponsors = dbContext.Sponsors
const fs = require('fs')
...
exports.findAll = async (req, res) => {
    try {
        ...
        if (search) {
            data = await Sponsors.findAndCountAll({
                distinct: true,
                offset,
                limit,
                where: {
...
exports.create = async (req, res) => {
    try {
        if (req.body.id == undefined) {
            await Sponsors.create(req.body);
        } else {
            await Sponsors.update(req.body, { where: { id: req.body.id } })
        }
...
exports.findById = async (req, res) => {
    try {
        const { id } = req.params
        const data = await Sponsors.findOne({
            where: { id },
        });
        return res.status(200).json({ data });
    }
...
exports.update = async (req, res) => {
    try {
        ...
        const [count, data] = await Sponsors.update(payload, {
