...
const CourseCategories = dbContext.CourseCategories
...
exports.findAll = async (req, res) => {
    try {
        ...
        if (search) {
            data = await CourseCategories.findAndCountAll({
                distinct: true,
                offset,
                limit,
                where: {
                    course_category: { [Op.like]: `%${search}%` }
...
exports.getAll = async (req, res) => {
    try {
        const data = await CourseCategories.findAll({
            include: [
                {
                    model: Courses,
                    requied: false,
...
exports.create = async (req, res) => {
    try {
        if (req.body.id == undefined) {
            await CourseCategories.create(req.body);
        } else {
            await CourseCategories.update(req.body, { where: { id: req.body.id } })
        }
        return res.status(200).json({ message: 'Course category created!' })
    }
    catch (err) {
        return res.status(500).json(err)
    }
...
exports.findById = async (req, res) => {
    try {
        const { id } = req.params
        const data = await CourseCategories.findOne({
            where: { id }
        });
        return res.status(200).json({ data });
    }
...
exports.update = async (req, res) => {
    try {
        ...
        const [count, data] = await CourseCategories.update(payload, {
